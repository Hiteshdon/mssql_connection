...
class NativeLoader {
  ...
  static DynamicLibrary loadCTLib() {
    ...
    if (Platform.isAndroid) {
      ...
      } catch (e) { lastErr = e; tried.add('ct.dll'); NativeLogger.w('Windows[CT]: ct.dll by name failed -> $e'); }
      try {
        final scriptDir = File.fromUri(Platform.script).parent;
        ...
        for (final dir in candidateRootDirs) {
          ...
          if (File(p).existsSync()) {
            _setDllDirectory(dir); NativeLogger.i('Windows[CT]: SetDllDirectory($dir)');
            _preloadWithAlteredSearchPath(p); NativeLogger.i('Windows[CT]: preload $p');
      ...
      try {
        ...
        final p = '$cwd\\ct.dll';
        if (File(p).existsSync()) {
          _setDllDirectory(cwd); NativeLogger.i('Windows[CT]: SetDllDirectory($cwd)');
          _preloadWithAlteredSearchPath(p); NativeLogger.i('Windows[CT]: preload $p');
          return DynamicLibrary.open(p);
        }
      } catch (_) {}
  ...
  static void _setDllDirectoryFor(String dllPath) {
    try {
      ...
      final setDllDir = k32.lookupFunction<Int32 Function(Pointer<Utf16>), int Function(Pointer<Utf16>)>('SetDllDirectoryW');
  ...
  static void _preloadWithAlteredSearchPath(String dllPath) {
    try {
      final k32 = DynamicLibrary.open('kernel32.dll');
      final loadLibraryEx = k32.lookupFunction<
      ...
      final h = loadLibraryEx(p, nullptr, 0x00000008);
      malloc.free(p);
